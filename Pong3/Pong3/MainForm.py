...
class MainForm(Form):
	...
	def TimerballTick(self, sender, e):
		...
		if ball.Location.X >= self.Width or (ball.Location.X > rpdl.Right + 20 and ball.Location.Y > rpdl.Top):
			...
			"""TO DO, finish rightscore condition"""
		...
		"""TODO: ?"""
		if self.timerboolean.Enabled == True:
			lpdl.Top = ball.Top - 20
			System.Random(sleep(2))
	...
	def reset():
		...
		#Need to set a function to reset easter eggs
		bl.BackColor = Color.Black
		self._lblleft.Color = Color.White
		self._lblright.Color = Color.White
		
		
	if e.KeyCode == Keys.R:
		reset()
	#make a secret control
	...
	if tdum.Enabled:
		if e.KeyCode == Keys.Up:
			...
			tright.Enabled = False
			#finish multi controlls
			
	if tmult.Enabled and tball.Enabled:	#these are multi controls
		if e.KeyCode == Keys.W:
			self.flagleft = False
			tleft.Enabled = True
			tbool.Enabled = False
	...
	def LblballClick(self, sender, e):
		...
		self._lblleft.Color = Color.Black
		self._lblright.Color = Color.Black
		self._lblleftscore.Color = Color.Black
		self._lblrightscore.Color = Color.Black
		#make ball go faster and be smaller
		"""Put more easter eggs here later"""
		pass

	def RightscoreClick(self, sender, e):
		self._rightscore = float(self._rightscore)
		self._rightscore += 0.01
		pass

	def LeftscoreClick(self, sender, e):
		self._leftscore = float(self._leftscore)
		self._leftscore += 0.01
		pass